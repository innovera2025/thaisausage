...
{"receipt_id": "DO2609040001", "receipt_key": "eVRP:DO2609040001",
 "source": "eVRP", "do_no": "DO2609040001", "transaction_no": 900001,
 "state": "staged", "erp_write": false}
```

ส่งซ้ำด้วยข้อมูลเดิมจะได้ก้อนเดิมพร้อม `"replayed": true`

### 4.1 `transaction_no` — เลขเอกสารใน ERP

| ฟิลด์ | คือ | เก็บไว้ทำอะไร |
|---|---|---|
| `receipt_id` | รหัสที่ **eVRP** ตั้งเอง | ใช้กันข้อมูลซ้ำ ระบบเราจำไว้ |
| `do_no` | เลขที่ DO | ใช้ระบุเอกสารตอนแก้ไขภายหลัง |
| `transaction_no` | **เลขเอกสารใน ERP** ที่ระบบเราออกให้ | เก็บไว้อ้างอิงเวลาคุยกับทีม ERP |

`transaction_no` จะเป็น `null` จนกว่าเอกสารจะถูกบันทึกเข้า ERP สำเร็จ ถ้าอยากรู้ภายหลังว่าใบไหนได้เลข
อะไร ส่ง payload เดิมซ้ำเข้ามาได้ จะได้คำตอบเดิมพร้อมเลขล่าสุดโดยไม่สร้างข้อมูลซ้ำ

**ไม่ต้องส่ง `transaction_no` กลับมา** ตอนแก้ไขเอกสาร — ระบบเราค้นเองจาก `do_no` เพื่อไม่ให้ค่าจาก
ภายนอกชี้ไปที่เอกสารใบอื่นใน ERP ได้
...
เมื่อ DO ที่เคยส่งมามีการเปลี่ยนแปลง (เปลี่ยนคนขับ เปลี่ยนวันส่ง แก้จำนวน เพิ่มหรือลดรายการ)
ให้ส่งมาที่เส้นทางนี้ **ไม่ใช่เส้นทางเดิม** เพราะเส้นทางเดิมจะตอบ `409` เมื่อข้อมูลเปลี่ยน

```
POST /api/v1/vrp/do-updated
Authorization: Bearer <THAISAUSAGE_API_KEY>
Content-Type: application/json
```
...
| ฟิลด์ | ค่าที่ต้องใส่ |
|---|---|
| `receipt_id` | **รหัสใหม่** คนละค่ากับตอนสร้าง เช่น เติม `-R2`, `-R3` ต่อท้าย |
| `do_no` | **เลขที่ DO เดิม** ที่ต้องการแก้ |
| `header` | หัวเอกสาร **ฉบับล่าสุดทั้งชุด** ไม่ใช่เฉพาะช่องที่เปลี่ยน |
| `details` | รายการสินค้า **ทั้งหมดที่ถูกต้อง ณ ตอนนี้** |

ระบบจะแทนที่ค่าทั้งหมดด้วยสิ่งที่ส่งมา — ช่องที่ไม่ส่งจะกลายเป็นว่าง และรายการสินค้าเดิมจะถูกแทนที่
ด้วยชุดใหม่ทั้งหมด จึงต้องส่งเอกสารฉบับสมบูรณ์เสมอ ไม่ใช่ส่งเฉพาะส่วนที่แก้

```json
{
  "receipt_id": "DO2609040001-R2",
  "do_no": "DO2609040001",
  "header": {"do_no": "DO2609040001", "dodate": "2026-09-18", "delivery_date": "2026-09-20",
             "cust_code": "CT-MS-BKK-2092", "driver": "สมชาย ใจดี", "car_number": "70-1234 กรุงเทพมหานคร"},
  "details": [{"item_code": "A20-002", "qty": 8, "units": "ลัง", "saleprice": 595.00, "amount": 4760.00}]
}
```
...
```json
{"receipt_id": "DO2609040001-R2", "do_no": "DO2609040001", "transaction_no": 900001,
 "state": "staged", "erp_write": true,
 "do_update": {"state": "updated", "header_rows": 1, "detail_rows": 1}}
```

| `do_update.state` | หมายความว่า |
|---|---|
| `updated` | แก้เรียบร้อยแล้ว |
| `rejected` + `reason: do_no_not_written` | ไม่พบ DO เลขนี้ใน ERP — ต้องสร้างด้วยเส้นทางข้อ 2 ก่อน |
| `rejected` + `reason` กล่าวถึง approved/closed | เอกสารถูกอนุมัติหรือปิดแล้ว แก้จากตรงนี้ไม่ได้ |
| `disabled` | ยังไม่เปิดใช้งาน ติดต่อทีม Thai Sausage |
| `needs_review` | ผลไม่แน่ชัด **ห้ามส่งซ้ำ** แจ้งทีม Thai Sausage |

### 10.3 กติกา

- **แก้ได้เฉพาะเอกสารที่ยังไม่ถูกอนุมัติ ปิด หรือลงบัญชี** — เมื่อฝ่ายบัญชีรับเอกสารแล้ว
  ระบบจะปฏิเสธการแก้ทุกกรณี ต้องติดต่อทีม ERP โดยตรง
- **ส่งซ้ำด้วย `receipt_id` เดิมและข้อมูลเดิม** ปลอดภัย ระบบไม่แก้ซ้ำ ตอบ `"replayed": true`
- **`receipt_id` เดิมแต่ข้อมูลต่าง** จะได้ `409` ให้ใช้รหัสใหม่
- **ไม่ต้องส่ง `transaction_no`** ระบบค้นจาก `do_no` เอง
- **การแก้ทั้งหมดเกิดในทรานแซกชันเดียว** ถ้าล้มเหลวตรงไหน เอกสารจะคงเดิมทั้งใบ ไม่มีสภาพครึ่งๆ กลางๆ
- **ข้อมูลจาก eVRP ถือเป็นฉบับจริง** ถ้าทีม ERP แก้เอกสารไว้ก่อน แล้ว eVRP ส่งฉบับแก้มาทีหลัง
  ข้อมูลจาก eVRP จะทับของเดิม (ยกเว้นเอกสารที่อนุมัติ ปิด หรือลงบัญชีแล้ว ซึ่งระบบจะปฏิเสธ)
- **การยกเลิก DO ทำได้เฉพาะทีม ERP เท่านั้น** ระบบนี้ไม่ยกเลิกเอกสารให้ไม่ว่ากรณีใด
  ถ้า DO ต้องถูกยกเลิก ให้ประสานทีม ERP โดยตรง
